...
export function createContactInput(data) {
  const select = createContactSelect(data);

  const contactField = el('.contact-field.field', [
    el('div.contact-field__wrapper', [
      el('input.contact-field__input', {
        type: 'text',
        name: data ? data.socialType : 'phone',
        'data-valid': data ? data.socialDataValid : 'phone',
        value: data ? data.socialLink : '',
        required: true,
      }),
      el('label', !data ? `+x (xxx) xxx-xx-xx` : ''),
      el('small'),
    ]),
    el(
      'button.contact-field__del-btn',
      { type: 'button', 'data-action': 'del-contact', hidden: Boolean(!data) },
      svg(
        'svg',
        {
          width: '12',
